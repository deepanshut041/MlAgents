...
class DDQNAgent():
    def __init__(self, input_shape, action_size, buffer_size, batch_size, gamma, lr, tau, update_every, device):
        ...
        # Q-Network
        self.policy_net = DDQNLinear(input_shape, action_size).to(self.device)
        self.target_net = DDQNLinear(input_shape, action_size).to(self.device)
    ...
    def act(self, states, eps=0.01):
        ...
        if random.random() > eps:
            return np.max(action_values.cpu().data.numpy(), axis=1)
        else:
            return np.array([random.choice(np.arange(self.action_size)) for s in states])
